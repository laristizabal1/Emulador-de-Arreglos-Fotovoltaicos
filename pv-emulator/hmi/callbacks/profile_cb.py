...
Callbacks registrados (2):
    - update_profile  -> calcula Vset/Iset/Pset y actualiza graficas + stats
...
def register(app):
    ...
    def update_profile(nasa_data, module_key, Ns, Np,
        ...
        # Construir ModuleParams desde catalogo o sliders custom
        ...
        fig_vi = go.Figure()
        fig_vi.add_trace(go.Scatter(x=labels, y=[d["V_set"] for d in profile],
            name="V (V)", line=dict(color=C["red"], width=2, shape="hv")))
        fig_vi.add_trace(go.Scatter(x=labels, y=[d["I_set"] for d in profile],
            name="I (A)", line=dict(color=C["blue"], width=2, shape="hv"), yaxis="y2"))
        fig_vi.update_layout(height=190, margin=dict(t=10, b=20, l=30, r=40),
        ...
        fig_pt = go.Figure()
        fig_pt.add_trace(go.Scatter(x=labels, y=[d["P_set"] for d in profile],
            name="P (W)", fill="tozeroy",
            line=dict(color=C["accent"], width=2),
            fillcolor="rgba(22,163,74,0.12)"))
        fig_pt.add_trace(go.Scatter(x=labels, y=[d["Tcell"] for d in profile],
            name="T_cell (C)", line=dict(color=C["cyan"], width=1.5), yaxis="y2"))
        fig_pt.update_layout(height=170, margin=dict(t=10, b=20, l=30, r=40),
            paper_bgcolor="white", plot_bgcolor="white",
            legend=dict(font=dict(size=9)),
            xaxis=dict(tickfont=dict(size=8), dtick=n_ticks),
